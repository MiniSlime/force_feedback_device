...
- 句読点は全角ピリオド（．）および全角カンマ（，）を使用する。

## 卒論執筆・LaTeX環境メモ

### 論文内容（現状の方針）
- テーマ: 複数の小型プロペラを用いた力覚提示の方向知覚特性の調査。
- 背景ストーリー: 小型デバイス（スマートウォッチ等）の方向提示は振動パターン中心で認知負荷や方向数に課題がある。一方で大型装置では持続的な力覚による分かりやすい提示が可能だが，小型化に不向き。そこで小型プロペラを用いた力覚提示を提案する。
- 概要に含める内容: 装着方式（wrist-worn / hand-grip）を用いた知覚特性の調査，装着方式が知覚特性と負荷に影響する示唆。

### LaTeX環境（執筆用テンプレート）
- テンプレート: `卒論/thesis-jp-utf8/`
  - `thesis-jp-utf8.tex`（本文テンプレート）
  - `mast-jp-utf8.sty`（表紙スタイル）
  - `sample.eps`（図のダミー）
- 参照元: `卒論/sample-jp-utf8/` と同じ書式で構成。

### ビルド設定（platex + dvipdfmx）
- 目的: `jreport` を使うため pLaTeX でビルドする。
- `卒論/thesis-jp-utf8/.latexmkrc`
  - `platex` / `jbibtex` / `dvipdfmx` を使用。
  - `force_mode` 有効化で初回 `.toc/.lof` 欠如を許容。
- ワークスペース設定: `/.vscode/settings.json`
  - `latexmk (platex)` レシピをデフォルト化。
  - `-f` を付けて初回の欠落ファイルで止まらないようにする。

### 既知の注意点
- `uplatex` で `JY1` エンコーディングエラーが出る場合があるため，本プロジェクトでは `platex` を使用する。
- BibTeXのスタイルは現在 `unsrt` を使用している（`junsrt.bst` が無い環境があるため）。

### 参考文献（BibTeX）追加手順と最終チェック
1. **情報抽出**：対象PDFから著者名，題名，会議／雑誌名，年，巻号，ページ，DOI，ISBN，開催地などを確認する（ACMなら「ACM Reference Format」を優先）。
2. **BibTeX追記**：`卒論/thesis-jp-utf8/references.bib` に適切なエントリ（`@inproceedings` など）を追加し，`citekey` を決める。
3. **本文反映**：`thesis-jp-utf8.tex` 側で `\cite{citekey}` を使用する。
4. **ビルド**：以下の順でビルドして `.bbl` を生成する。  
   - `bibtex thesis-jp-utf8`  
   - `platex thesis-jp-utf8.tex` ×2  
5. **最終チェック**：  
   - `thesis-jp-utf8.bbl` が空でないこと  
   - 本文の引用が `[?]` でないこと  
   - 参考文献一覧に追加分が表示されること  
   - 著者順，題名，年，DOI等の表記がPDFの情報と一致すること  
