...
duty_lrt <- anova(no_duty_model, full_model, test = "Chisq")
test_df <- data.table(
  test = "dutyCycle_main_effect",
  df = duty_lrt[2, "Df"],
  chi_sq = duty_lrt[2, "Chisq"],
  p_value = duty_lrt[2, "Pr(>Chisq)"]
)
...
interaction_dt <- interaction_dt[term %in% c("dutyCycle:method", "dutyCycle:direction")]
...
output_path <- file.path(output_dir, "task_accuracy_glmm_duty.csv")
