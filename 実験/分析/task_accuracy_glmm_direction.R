...
main_model <- glmer(
  isCorrect_new ~ direction + method + dutyCycle + (1 | participantId),
  data = raw_dt,
  family = binomial,
...
no_direction_model <- glmer(
  isCorrect_new ~ method + dutyCycle + (1 | participantId),
  data = raw_dt,
  family = binomial,
  control = glmerControl(optimizer = "bobyqa")
)
...
interaction_model <- glmer(
  isCorrect_new ~ direction * method + direction * dutyCycle + (1 | participantId),
  data = raw_dt,
  family = binomial,
  control = glmerControl(optimizer = "bobyqa")
)
