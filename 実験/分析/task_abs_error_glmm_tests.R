...
additive_model <- lmer(
  absError ~ method + dutyCycle + direction + (1 | participantId),
  data = raw_dt,
  REML = FALSE
)

no_method_model <- lmer(
  absError ~ dutyCycle + direction + (1 | participantId),
  data = raw_dt,
  REML = FALSE
)

no_duty_model <- lmer(
  absError ~ method + direction + (1 | participantId),
  data = raw_dt,
  REML = FALSE
)

no_direction_model <- lmer(
  absError ~ method + dutyCycle + (1 | participantId),
  data = raw_dt,
  REML = FALSE
)
...
output_path <- file.path(output_dir, "task_abs_error_glmm_tests.csv")
